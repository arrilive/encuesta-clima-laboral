...
- **Retención de datos en el proveedor de SMS (Twilio):** Twilio almacena temporalmente el número de destino, el código OTP y el enlace de acceso en su consola de Messaging Logs como parte de la entrega del mensaje — inherente a cómo funciona el canal SMS con cualquier proveedor externo. Se mitiga parcialmente configurando `contentRetention=discard` y `addressRetention=obfuscate` en cada envío (implementado), lo que reduce la ventana de exposición del contenido y el número en la consola de Twilio. La retención residual y su configuración de ventana (7-400 días) se administra directamente en la consola de Twilio, fuera del alcance de esta aplicación. Adicionalmente, mientras la encuesta de una persona no esté marcada como completada, el enlace de acceso funciona como credencial portadora: quien lo tenga (incluyendo alguien con acceso a la consola de Twilio antes de que expire la ventana de retención) puede ver y modificar las respuestas dadas hasta ese momento — específicamente las de esa persona, sin que esto comprometa el anonimato del resto del sistema. Una vez completada la encuesta, el token queda inactivo y el acceso se bloquea por completo.
